...
class InputHandler {
  ...
  constructor(canvas, scene, shapeObject) {
    ...
    //Texture
    document.getElementById("textureType").onchange = function() {_inputHandler.readTexture()};
  ...
  click(ev) {
    ...
    if (this.shapeObject == 1) {
      var shape = new Triangle(shader, ev.clientX, ev.clientY, r, g, b, shapeMultiplier, _inputHandler.isRainbow);
    } else if (this.shapeObject == 2) {
      var shape = new spinningSquare(shader, ev.clientX, ev.clientY, r, g, b, shapeMultiplier, _inputHandler.isRainbow);
    } else if (this.shapeObject ==3) {
      var shape = new movingCircles(shader, ev.clientX, ev.clientY, circlePointNumber, r, g, b, shapeMultiplier, _inputHandler.isRainbow);
    } else {
