...
class movingCircles extends Geometry {
  ...
  generateCircleVertices(xPos, yPos, pointVal, redVal, greenVal, blueVal, shapeMultiplier, isRainbow) {
    ...
    for (var i = 0; i <= pointVal; i++) {
      ...
      if (isRainbow) {
        var vertexPoint = new Vertex(x, y, 0, Math.random(), Math.random(), Math.random());
      } else {
      ...
      if (isRainbow) {
        vertices.push(new Vertex(xPos, yPos, 0, Math.random(), Math.random(), Math.random()))
        vertices.push(new Vertex(x, y, 0, Math.random(), Math.random(), Math.random()));
      } else {
